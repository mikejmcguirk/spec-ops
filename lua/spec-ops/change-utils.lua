...
local function chg_chars(op_state)
	...
	--- @type integer|nil, integer|nil, string|nil
	local l_byte, _, bb_err = blk_utils.byte_bounds_from_col(op_state.start_line_post, start_col_post)
	...
	vim.api.nvim_buf_set_mark(0, "[", op_state.marks_post.start.row, op_state.marks_post.start.col, {})
...
local function do_block_chg_ops(op_state, chg_info)
	...
	local function exec(i, o)
		...
		if i == 1 then
			...
			--- @type integer|nil, integer|nil, string|nil
			local l_byte, _, err = blk_utils.byte_bounds_from_col(op_state.start_line_post, start_byte)
		...
		if i == #chg_info then
			...
			if fin_vcol < start_l_vcol then
				--- @type integer|nil, integer|nil, string|nil
				local l_byte, _, err = blk_utils.byte_bounds_from_vcol(op_state.start_line_post, fin_vcol)
				if (not l_byte) or err then
			...
			--- @type integer|nil, integer|nil, string|nil
			local l_byte, _, err = blk_utils.byte_bounds_from_vcol(op_state.fin_line_post, fin_vcol)
			if (not l_byte) or err then
				return "do_block_ops: " .. (err or "Unknown error in byte_bounds_from_vcol")
			end
