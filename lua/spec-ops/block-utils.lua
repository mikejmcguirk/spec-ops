...
local function is_valid_col(line, col)
	...
	if #line == 0 and col > 0 then
		--- @type string
		local err_msg = string.format("is_valid_col: Input col %d greater than zero on a zero length line", col)
...
function M.byte_bounds_from_col(line, col)
	...
	if char_idx == -1 then
		--- @type string
		local err_msg = string.format("byte_bounds_from_col: charidx() returned -1 for line %s and col %d", line, col)
	...
	if start_byte == -1 then
		--- @type string
		local err = string.format("byte_bounds_from_col: byteidx() returned -1 for line %s and col %d", line, col)
		return nil, nil, err
	end
	...
	if char == "" then
		--- @type string
		local err = string.format('byte_bounds_from_col: strcharpart() returned "" for line %s and col %d', line, col)
		return nil, nil, err
	end
...
function M.vcols_from_char_idx(line, char_idx, char, char_len)
	...
	if char_idx >= char_len then
		--- @type string
		local err_msg = string.format("vcols_from_char_idx: char_idx %d >= char_len %d", char_idx, char_len)
		return nil, nil, err_msg
	end
	...
	if byte_start == -1 then
		--- @type string
		local err_msg =
			string.format("vcols_from_char_idx: byteidx() returned -1 for char_idx %d into string %s", char_idx, line)
		return nil, nil, err_msg
	end
...
function M.vcols_from_vcol(line, vcol)
	...
	--- @type string
	local err_msg = string.format("vcols_from_vcol: Unable to find vcols for vcol %d in line %s", vcol, line)
	return nil, nil, err_msg
end
...
function M.byte_bounds_from_char_idx(line, char_idx, char, char_len)
	...
	if char_idx >= char_len then
		local err_msg =
			string.format("byte_bounds_from_char_idx: char_idx %d greater than character length %d", char_idx, char_len)
		return nil, nil, err_msg
	end
	...
	if start_byte == -1 then
		--- @type string
		local err_msg =
			string.format("byte_bounds_from_char_idx: byteidx of -1 for char_idx %d in line %s", char_idx, line)
		return nil, nil, err_msg
	end
...
function M.vcols_from_byte_bounds(line, start_byte, end_byte)
	...
	if start_byte > end_byte then
		--- @type string
		local err_msg = string.format("vcols_from_byte_bounds: Start byte %d > end byte %d", start_byte, end_byte)
		return nil, nil, err_msg
	end
	...
	if (not valid_end_byte) or end_byte_err then
		--- @type string
		local err_msg = "invalid end byte at vcols_from_byte_bounds: " .. (end_byte_err or "nil error at is_valid_col")
		return nil, nil, err_msg
	end
...
function M.byte_bounds_from_vcol(line, vcol)
	...
	if vcol < 0 or vcol > max_vcol then
		--- @type string
		local err = string.format("byte_bounds_from_vcol: Invalid vcol %d. Max vcol %d", vcol, max_vcol)
		return nil, nil, err
	end
	...
	while low_idx <= high_idx do
		...
		if start_vcol <= vcol and vcol <= end_vcol then
			--- @type integer|nil, integer|nil, string|nil
			local start_byte, end_byte, bb_err = M.byte_bounds_from_char_idx(line, mid_idx, char, char_len)
			if (not start_byte) or not end_byte or bb_err then
	...
	while idx < char_len do
		...
		if start_vcol <= vcol and vcol <= end_vcol then
			--- @type integer|nil, integer|nil, string|nil
			local start_byte, end_byte, bb_err = M.byte_bounds_from_char_idx(line, idx, char, char_len)
			if (not start_byte) or not end_byte or bb_err then
				return nil, nil, "byte_bounds_from_vcol: " .. bb_err
			end
			return start_byte, end_byte, nil
	...
	--- @type string
	local err_msg = string.format("byte_bounds_from_vcol: Unable to find vcols for vcol %d in line %s", vcol, line)
	return nil, nil, err_msg
end
